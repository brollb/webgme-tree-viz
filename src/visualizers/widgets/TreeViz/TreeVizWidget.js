...
define([
  ...
  TreeVizWidget.prototype._initialize = function () {
    ...
    this._svg = this.__svg.append("g").attr(
      "transform",
  ...
  TreeVizWidget.prototype._update = function (source) {
    ...
    var nodeEnter = node.enter().append("g")
      ...
      .attr("transform", function (d) {
        return d.parent
          ? "translate(" + d.parent.y0 + "," + d.parent.x0 + ")"
          : "translate(" + d.y + "," + d.x + ")";
      })
      .on("click", function (d) {
    ...
    if (this.__resizing !== true) {
      ...
      this._resizeD3Tree(
        nodesYByDepth.length * TREE_LEVEL_DISTANCE,
        sum * NODE_SIZE,
      );
  ...
  TreeVizWidget.prototype.setData = function (data) {
    ...
    if (this._root && this._root.name) {
      ...
      if (this._root.children) {
        rootNameWidth =
          (this._root.name + " [" + this._root.children.length + "]").length *
          6;
      }
      this._el.find("svg > g").attr(
        "transform",
        "translate(" + (rootNameWidth + MARGIN) + "," + MARGIN + ")",
      );
    }
  };
